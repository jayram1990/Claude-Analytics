(function() {
    ...
    template.innerHTML = `
        <style>
            ...
            .chat-header { padding: 14px 18px; background: #0f172a; color: #ffffff; font-weight: 600; font-size: 14px; display: flex; align-items: center; justify-content: space-between; }
            .provider-badge { font-size: 10px; font-weight: 400; padding: 2px 8px; border-radius: 9999px; background: rgba(255,255,255,0.15); }
            ...
            button { background: #2563eb; color: #ffffff; border: none; border-radius: 50%; width: 36px; height: 36px; margin-left: 10px; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
            button:disabled { background: #cbd5e1; cursor: not-allowed; }
        </style>
        <div class="chat-container">
            <div class="chat-header">
                <span id="headerDom">Claude Analytics</span>
                <span class="provider-badge" id="providerBadge">Not configured</span>
            </div>
            <div class="chat-area" id="boxDom"></div>
            <div class="input-area">
                <input type="text" id="inputDom" placeholder="Configure API URL and Key to start..." disabled />
                <button id="btnDom" disabled>&#x27A4;</button>
            </div>
        </div>
    `;

    // Maps Anthropic model IDs → OpenRouter model IDs for auto-conversion
    ...
    class ClaudeRuntimeWidget extends HTMLElement {
        constructor() {
            ...
            this.$providerBadge = this._shadowRoot.getElementById("providerBadge");
        ...
        connectedCallback() {
            // SAC React store subscription — mirror layout component metadata up to canvas
            try {
                if (window.commonApp) {
                        ...
                        let processReactStore = state => {
                            ...
                            if (metaString !== this.metadata) {
                                ...
                                this.dispatchEvent(new CustomEvent("propertiesChanged", {
                                    detail: { properties: { metadata: metaString } }
                                }));
                            }
                        };
        ...
        onCustomWidgetAfterUpdate(changedProperties) {
            ...
            if (changedProperties["apiUrl"] || changedProperties["apiKey"]) {
                this._settings.apiUrl = changedProperties["apiUrl"] ?? this._settings.apiUrl;
                this._settings.apiKey = changedProperties["apiKey"] ?? this._settings.apiKey;
        ...
        _refreshProviderState() {
            const url = this._settings.apiUrl || "";
            ...
            if (url.includes("anthropic.com")) {
                ...
                this.$providerBadge.innerText = "Custom Proxy";
            } else {
                this.$providerBadge.innerText = "Not configured";
            }

            if (ready) {
                this.$inputDom.placeholder = "Ask Claude about your data...";
            } else {
                this.$inputDom.placeholder = "Configure API URL and Key to start...";
        ...
        setTableData(dataString) {
            if (dataString && dataString.length > 0) {
                ...
                // Only enable input once both data AND credentials are present
                const ready = this._settings.apiUrl && this._settings.apiKey;
                this.$inputDom.disabled = !ready;
                this.$btnDom.disabled   = !ready;
                this.$inputDom.placeholder = ready
                    ? "Ask Claude about your data..."
                    : "Configure API URL and Key to start...";
        ...
        async executePrompt() {
            ...
            this.printMessage("system-msg", "Claude is thinking...");
            ...
            try {
                ...
                // Resolve model ID for the target provider
                let modelId = this._settings.model;
                if (isOpenRouter && !modelId.includes("/")) {
                    // Auto-convert Anthropic model IDs to OpenRouter format
                    modelId = ANTHROPIC_TO_OPENROUTER[modelId] || `anthropic/${modelId}`;
                }
                ...
                if (!response.ok) {
                    ...
                    this.printMessage("system-msg", `API error ${response.status}: ${response.statusText}${errBody ? " — " + errBody.slice(0, 120) : ""}`);
                    return;
                }

                const payload = await response.json();

                // Handle Anthropic format (content[0].text) and OpenAI/OpenRouter format (choices[0].message.content)
                ...
                if (responseText) {
                    ...
                    const errorDetail = payload.error ? payload.error.message : JSON.stringify(payload).slice(0, 120);
                    this.printMessage("system-msg", `Unexpected response: ${errorDetail}`);
                }
            } catch (err) {
                if (this.$boxDom.lastChild && this.$boxDom.lastChild.classList.contains("system-msg")) {
                    this.$boxDom.removeChild(this.$boxDom.lastChild);
                }
                this.printMessage("system-msg", `Request failed: ${err.message}`);
            }
        }
